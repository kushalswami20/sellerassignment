...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await fetch('https://sellerassignment.onrender.com/:productId', {
          method: 'POST',
          headers: {
        ...
        if (data.success) {
          ...
          console.log('Image Data:', data.product.img); // Log the entire img array
  ...
  const debugImageData = (imgArray) => {
    if (!imgArray || !Array.isArray(imgArray) || imgArray.length === 0) {
      console.error('Invalid image array:', imgArray);
      return null;
    }
    ...
    try {
      ...
      if (!firstImage.data) {
        console.error('No image data found:', firstImage);
        return null;
      }
  
      const base64String = Buffer.from(firstImage.data.data || firstImage.data).toString('base64');
      return `data:${firstImage.contentType};base64,${base64String}`;
    } catch (error) {
  ...
  const arrayBufferToBase64 = (imgArray) => {
    if (!imgArray || !Array.isArray(imgArray) || imgArray.length === 0) {
      console.error('Invalid image array:', imgArray);
      return null;
    }
    ...
    try {
      ...
      let binary = '';
      const bytes = new Uint8Array(firstImage.data.data || firstImage.data);
      const len = bytes.byteLength;
      for (let i = 0; i < len; i++) {
        binary += String.fromCharCode(bytes[i]);
      }
      return `data:${firstImage.contentType};base64,${window.btoa(binary)}`;
    } catch (error) {
      console.error('Error converting image:', error);
      return null;
    }
  ...
  const fetchRelatedProducts = async (category) => {
    try {
      const response = await fetch(`https://sellerassignment.onrender.com/product/category/${category}`);
      if (!response.ok) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const response = await fetch('https://sellerassignment.onrender.com/cart/addtocart', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
              ...
              <div className="p-8 bg-gray-50 flex items-center justify-center">
                ...
                >
                  <img
                    src={arrayBufferToBase64(product.img)}
                    alt={product.name}
           ...
           <div className="mt-12">
            ...
            <p className="text-gray-600 w-full truncate overflow-hidden text-ellipsis"
  style={{ maxWidth: "100%", wordBreak: "break-word", whiteSpace: "normal" }}>{product.description}</p>
            </div>
          {/* Reviews Section */}
          <div className="mt-12">
