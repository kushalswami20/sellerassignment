  ...
  const ProductImage = ({ imageData }) => {
    ...
    useEffect(() => {
      ...
      if (typeof imageData === 'string' && imageData.startsWith('http')) {
        ...
        setImgSrc(imageData);
      } else if (imageData.type === 'Buffer' || imageData instanceof ArrayBuffer) {
        // If it's an ArrayBuffer or Buffer, convert it
        const base64 = arrayBufferToBase64(imageData.data || imageData);
        console.log('Base64:', base64);
        setImgSrc(`data:image/jpeg;base64,${base64}`);
      }
    }, [imageData]);
  
    if (!imgSrc) return <div className="w-full h-48 bg-gray-200 animate-pulse" />;
    ...
    >
      <Link to={`/${product.productId}`} className="block">
        <div className="aspect-w-1 aspect-h-1 w-full">
          <ProductImage imageData={product.img} />
        </div>
        <div className="p-4">
      ...
      {categories.map((category, index) => (
        ...
        >
          <div className="aspect-w-16 aspect-h-9">
            <ProductImage imageData={category.img} />
          </div>
