...
const CartItems = () => {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      ...
      try {
        ...
        const cartResponse = await fetch('https://sellerassignment.onrender.com/cart/get-cart', {
          method: 'POST',
          headers: {
        ...
        const productPromises = uniqueProductIds.map(async (productId) => {
          const productResponse = await fetch('https://sellerassignment.onrender.com/:productId', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
  ...
  const handleQuantityChange = async (itemId, change) => {
    ...
    if (newQuantity >= 1) {
      ...
      try {
        const userId = sessionStorage.getItem('userId');
        const response = await fetch('https://sellerassignment.onrender.com/cart/update-quantity', {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  const handleRemoveItem = async (itemId) => {
    ...
    try {
      const userId = sessionStorage.getItem('userId');
      const response = await fetch('https://sellerassignment.onrender.com/cart/delete-items', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleVoucherRedeem = async () => {
    try {
      const response = await fetch('https://sellerassignment.onrender.com/coupon/verify-coupon', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
