...
const AdminSignup = () => {
  ...
  const handleSignup = async () => {
    ...
    try {
      const response = await fetch('https://sellerassignment.vercel.app/admin/seller/signup', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setSellerId(data.sellerId); // Store the sellerId from the response
  ...
  const handleVerificationMethodSelect = async (method) => {
    ...
    try {
      const response = await fetch('http://localhost:5001/admin/send-otp', {
        ...
        body: JSON.stringify({ 
          sellerId // Changed from emailId to sellerId
        })
      });
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5001/admin/send-otp', {
        ...
        body: JSON.stringify({ sellerId }) // Changed from emailId to sellerId
      });
  ...
  return (
    <>
      ...
      <Navbar />
      <div className="min-h-screen flex items-center justify-center p-4 mt-16  bg-gradient-to-br from-pink-50 to-pink-100 px-4 py-8 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          >
            <div className="p-4 sm:p-6 lg:p-8">
              ...
              {successMessage && (
                <div className="bg-green-50 border border-green-200 text-green-600 px-3 py-2 sm:px-4 sm:py-3 rounded-lg mb-4 sm:mb-6 text-center text-sm sm:text-base">
                  {successMessage}
                </div>
              )}
