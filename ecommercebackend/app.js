...
// Middleware
app.use(cors({
  origin: ['http://localhost:3000','https://merabestie.com','https://hosteecommerce.vercel.app','https://sellerassignment-frontend.vercel.app'], 
  credentials: true,
...
app.post('/add-product', upload.array('images', 5), async (req, res) => {
  try {
    const { name, price, category, rating, inStockValue, soldStockValue,description } = req.body;
    ...
    // Generate unique productId (you can modify this format)
...
app.get('/product/category/:category', async (req, res) => {
  try {
    ...
    const product = await Product.findOne({ category: productId });
...
app.delete('/delete-product/:productId', async (req, res) => {
  try {
    const productId = req.params.productId; // Get the productId from the request params
    console.log('Received productId:', productId); // Debug log

    // Find and delete the product using the custom productId field
    const deletedProduct = await Product.findOneAndDelete({ productId: productId });

    if (!deletedProduct) {
      console.log('No product found with productId:', productId); // Debug log
      return res.status(404).json({
        success: false,
        message: 'Product not found',
      });
    ...
    console.log('Successfully deleted product:', deletedProduct); // Debug log
    res.status(200).json({
      success: true,
...
app.post("/place-order", async (req, res) => {
  try {
    ...
    const payload = {
      ...
      order_date: order.date, // Ensure this is in 'YYYY-MM-DD' format
      pickup_location: "Warehouse", // Update as per your Shiprocket settings
      billing_customer_name: order.name,
      billing_last_name: "", // Optional
      billing_address: order.address,
      billing_address_2: "",
      billing_city: "City", // Replace with the actual city
      billing_pincode: "123456", // Replace with the correct PIN code
      billing_state: "State", // Replace with the actual state
      billing_country: "India", // Replace if not India
      billing_email: order.email,
      billing_phone: "9876543210", // Replace with actual phone number
      ...
      order_items: order.productIds.map(productId => ({
        name: "Product Name", // Replace with the actual product name
        sku: productId,
        units: 1, // Number of units
        selling_price: order.price / order.productIds.length, // Divide price equally if multiple items
        discount: "",
        tax: "",
        hsn: ""
      })),
      payment_method: "Prepaid", // Replace with 'COD' for Cash on Delivery
      sub_total: order.price,
      length: 10, // Package dimensions
      breadth: 10,
      height: 10,
      weight: 1.5 // Replace with actual weight in kg
    };

    // Authenticate with Shiprocket to get the API token
    const authResponse = await axios.post("https://apiv2.shiprocket.in/v1/external/auth/login", {
      email: "kushalswami56@gmail.com", // Replace with your Shiprocket email
      password: "King123@" // Replace with your Shiprocket password
    });
    ...
    const trackingId = response.data.shipment_id;

    // Update the tracking ID in MongoDB
    await Order.updateOne({ orderId }, { trackingId });
